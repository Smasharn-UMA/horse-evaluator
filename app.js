...
function detail(id){const h=state.horses.find(x=>x.id===id),m=h.measurements||{},d=(l,v)=>`<div class="detail-item"><span>${l}</span><b>${esc(v)||'―'}</b></div>`;E.detailContent.innerHTML=`<div class="dialog-head"><div><div class="eyebrow">${h.year} ${esc(h.club)} ${h.horseNo!=null?'No.'+h.horseNo:''}</div><h2>${esc(h.name)}</h2></div></div>${h.photoUrl?`<div class="detail-photo" style="background-image:url('${esc(h.photoUrl)}')"></div>`:''}<div class="detail-grid">${d('性別',h.sex)}${d('生年月日',h.birthDate)}${d('父',h.sire)}${d('母',h.dam)}${d('母父',h.broodmareSire)}${d('所属',h.stableArea)}${d('厩舎',h.trainer)}${d('生産牧場',h.breeder)}${d('育成牧場',h.trainingFarm)}${d('内部管理ID',h.internalId)}${d('募集総額',h.price==null?'':money(h.price)+'万円')}${d('募集口数',h.shareCount==null?'':money(h.shareCount)+'口')}${d('1口価格',h.sharePrice==null?'':money(h.sharePrice)+'円')}${d('馬体重',m.weight==null?'':m.weight+'kg')}${d('体高',m.height==null?'':m.height+'cm')}${d('胸囲',m.girth==null?'':m.girth+'cm')}${d('管囲',m.cannon==null?'':m.cannon+'cm')}</div>${h.sourceUrl?`<a class="source-link" href="${esc(h.sourceUrl)}" target="_blank" rel="noopener">募集馬ページを開く</a>`:''}${h.videoUrl?`<p><a href="${esc(h.videoUrl)}" target="_blank" rel="noopener">歩様動画を開く</a></p>`:''}<h3>募集時PR</h3><div class="notes">${esc(h.recruitmentPr)||'―'}</div><h3>メモ</h3><div class="notes">${esc(h.notes)||'―'}</div><h3>更新履歴</h3><div class="notes">${(h.changeLog||[]).slice().reverse().map(x=>fmt(x.at)+'　'+esc(x.action)).join('\n')}</div>`;E.detailDialog.showModal()}
...
price:numberFrom(x,/総額\s*([0-9,]+)万円/),shareCount:numberFrom(x,/募集口数\s*([0-9,]+)口/),sharePrice:numberFrom(x,/一口価格\s*([0-9,]+)円/),recruitmentPr:first(x,/募集時のPR[\s\S]*?ポイント\s*([\s\S]*?)(?=生産者からのPR|主な兄姉・近親|$)/)||first(x,/募集時のPR\s*([\s\S]*?)(?=生産者からのPR|主な兄姉・近親|$)/),
measurements:{weight:latest(all('馬体重','kg')),height:latest(all('体高','cm')),girth:latest(all('胸囲','cm')),cannon:latest(all('管囲','cm'))},sourceUrl:url};if(data.sharePrice==null&&data.price!=null&&data.shareCount){data.sharePrice=Math.round((data.price*10000/data.shareCount)/100)*100}data.internalId=internalId(data.club,data.year,data.horseNo,'');return data}
...
function applyImported(d){const fields=['year','club','horseNo','name','sex','birthDate','sire','dam','broodmareSire','stableArea','trainer','breeder','price','shareCount','sharePrice','recruitmentPr','sourceUrl'];fields.forEach(k=>{if(d[k]!==''&&d[k]!=null)$(k).value=d[k]});['weight','height','girth','cannon'].forEach(k=>{if(d.measurements?.[k]!=null)$(k).value=d.measurements[k]})}
function importedCount(d){return[d.horseNo,d.name,d.sire,d.dam,d.trainer,d.price,d.shareCount,d.sharePrice,d.measurements.weight].filter(v=>v!==''&&v!=null).length}
