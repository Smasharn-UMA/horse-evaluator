...
(()=>{'use strict';
const K='horseEvaluator3',V='3.1.38',SCHEMA_VERSION=5,UI_K='horseEvaluator3_ui',PRE_IMPORT_K='horseEvaluator3_preImportBackup',PHOTO_DB='horseEvaluator3_photos',PHOTO_STORE='photos',VIDEO_DB='horseEvaluator3_videos',VIDEO_STORE='videos',$=id=>document.getElementById(id);let state;
  ...
  return {
    ...
    horses:a.map((h,i)=>{
      ...
      return {
        ...
        videoUrl:t(pick(h.videoUrl,h.video,h.files?.video)),...normalizeVideos(h),favorite:Boolean(h.favorite),notes:t(pick(h.notes,h.memo)),management:{owned:Boolean(h.management?.owned??h.ownedHorse),comparisonExcluded:Boolean(h.management?.comparisonExcluded??h.comparisonExcluded),tags:Array.isArray(h.management?.tags)?h.management.tags.map(t).filter(Boolean):Array.isArray(h.tags)?h.tags.map(t).filter(Boolean):[]},teacherData:{enabled:Boolean(h.teacherData?.enabled??h.isTeacherData),rank:t(h.teacherData?.rank??h.teacherRank),registeredAt:t(h.teacherData?.registeredAt),finalRating:scoreValue(h.teacherData?.finalRating??h.finalRating),satisfaction:n(h.teacherData?.satisfaction??h.satisfaction),retrospective:t(h.teacherData?.retrospective??h.retrospective),comment:t(h.teacherData?.comment??h.teacherComment)},evaluation:normalizeEvaluation(h.evaluation),
...
function list(){const y=E.yearFilter.value,c=E.clubFilter.value,sx=E.sexFilter.value,a=E.stableAreaFilter.value,q=E.searchInput.value.trim().toLowerCase(),fav=E.favoriteOnly.checked,s=E.sortSelect.value,view=E.dataViewFilter?.value||'all';return state.horses.filter(h=>(!y||String(h.year)===y)&&(!c||h.club===c)&&(!sx||h.sex===sx)&&(!a||h.stableArea===a)&&(!fav||h.favorite)&&(view==='all'||(view==='teacher'&&h.teacherData?.enabled)||(view==='recruitment'&&!h.teacherData?.enabled)||(view==='owned'&&h.management?.owned)||(view==='excluded'&&h.management?.comparisonExcluded))&&(!q||[h.horseNo,h.name,h.club,h.sire,h.dam,h.broodmareSire,h.stableArea,h.trainer,h.breeder,h.trainingFarm,h.currentLocation,h.horseClass,h.internalId,h.notes,(h.management?.tags||[]).join(' '),h.teacherData?.retrospective].join(' ').toLowerCase().includes(q))).sort((x,z)=>s==='updatedAt'?new Date(z.updatedAt)-new Date(x.updatedAt):s==='name'?x.name.localeCompare(z.name,'ja'):s==='priceDesc'?(z.price??-1)-(x.price??-1):s==='priceAsc'?(x.price??Number.MAX_SAFE_INTEGER)-(z.price??Number.MAX_SAFE_INTEGER):s==='sharePriceAsc'?(x.sharePrice??Number.MAX_SAFE_INTEGER)-(z.sharePrice??Number.MAX_SAFE_INTEGER):(x.horseNo??9999)-(z.horseNo??9999)||x.name.localeCompare(z.name,'ja'))}
function metric(l,v,u){return`<div class="metric"><b>${v==null?'―':esc(v)}</b><span>${l}${v==null?'':' '+u}</span></div>`}
function card(h){const m=h.measurements||{},mp=mainPhoto(h),bg=mp?`style="background-image:url('${esc(mp)}')"`:'',r=recommendation(h);return`<article class="horse-card"><div class="photo" ${bg}>${mp?'':'NO PHOTO'}</div><div class="body"><div class="topline"><div><span class="badge">${h.year} ${esc(h.club)}</span>${h.horseNo!=null?`<span class="badge">No.${h.horseNo}</span>`:''}${h.stableArea?`<span class="badge">${esc(h.stableArea)}</span>`:''}${h.teacherData?.enabled?`<span class="badge teacher-badge">教師データ${h.teacherData.rank?' '+esc(h.teacherData.rank):''}</span><span class="badge completeness-badge">充足 ${teacherCompleteness(h).percent}%</span>`:''}${h.management?.owned?`<span class="badge owned-badge">出資馬</span>`:''}<h3>${esc(h.name)}</h3><div class="horse-meta">${esc(h.sex)||'性別未設定'}${h.coatColor?'・'+esc(h.coatColor):''}${h.breeder?' ／ '+esc(h.breeder):''}</div>${(()=>{const q=evaluationAverage(h.evaluation);return q.average==null?'':`<div class="horse-meta">評価平均 <b>${q.average.toFixed(2)}</b>（${q.count}項目）</div>`})()}${r.score==null?'':`<div class="decision-strip"><div class="recommend-badge ${r.className}">${r.grade}</div><div><span>総合点</span><strong>${r.score.toFixed(1)}</strong></div><div><span>自信度</span><strong>${r.confidence}%</strong></div></div>`}</div><button class="favorite ${h.favorite?'on':''}" data-a="favorite" data-id="${h.id}">★</button></div><div class="pedigree">父：${esc(h.sire)||'―'}<br>母：${esc(h.dam)||'―'}<br>母父：${esc(h.broodmareSire)||'―'}<br>厩舎：${esc(h.trainer)||'―'}</div><div class="price-box"><b>総額 ${h.price==null?'―':money(h.price)+'万円'}</b><span>${h.shareCount==null?'募集口数 ―':money(h.shareCount)+'口'} ／ 1口 ${h.sharePrice==null?'―':money(h.sharePrice)+'円'}</span></div><div class="metrics">${metric('体重',m.weight,'kg')}${metric('体高',m.height,'cm')}${metric('胸囲',m.girth,'cm')}${metric('管囲',m.cannon,'cm')}</div><div class="card-actions"><button data-a="detail" data-id="${h.id}">詳細</button><button data-a="edit" data-id="${h.id}">編集</button><button data-a="teacher" data-id="${h.id}">${h.teacherData?.enabled?'教師解除':'教師追加'}</button><button data-a="delete" data-id="${h.id}" class="danger">削除</button></div><div class="updated">更新：${fmt(h.updatedAt)}</div></div></article>`}
...
function parseCarrotActive(text,url=''){
 ...
 const recordMatch=x.match(/(?:^|\n)\s*(平地|障害|地方)?\s*([A-Za-z0-9ⅠⅡⅢ]+)\s*[（(]\s*(\d+)\s*[-－]\s*(\d+)\s*[-－]\s*(\d+)\s*[-－]\s*(\d+)\s*[）)]/m);
...
function teacherLabel(h){const a=t(h.evaluation?.achievement);return{achievement:a,isG1Winner:a==='GⅠ勝利',isGradedWinner:a==='GⅠ勝利'||a==='重賞勝利',isOpenClass:['GⅠ勝利','重賞勝利','重賞出走','オープン'].includes(a)}}
function teacherDatasetObject(){const horses=(state.horses||[]).filter(h=>h.teacherData?.enabled).map(h=>({
 id:h.id,name:h.name,club:h.club,year:h.year,sex:h.sex,birthDate:h.birthDate,sire:h.sire,dam:h.dam,broodmareSire:h.broodmareSire,trainer:h.trainer,breeder:h.breeder,price:h.price,sharePrice:h.sharePrice,
...
function teacherAnalysis(){const ds=teacherDatasetObject(),rows=ds.horses;if(!rows.length)throw new Error('教師データが登録されていません。');const winners=rows.filter(x=>x.label.isGradedWinner),others=rows.filter(x=>!x.label.isGradedWinner&&x.label.achievement);const fields=EVAL_FIELDS.map(k=>{const a=winners.map(x=>x.features[k]).filter(v=>v!=null),b=others.map(x=>x.features[k]).filter(v=>v!=null),am=mean(a),bm=mean(b),pooled=(a.length>1&&b.length>1)?Math.sqrt(((a.length-1)*(stddev(a)||0)**2+(b.length-1)*(stddev(b)||0)**2)/(a.length+b.length-2)):null;return{key:k,label:EVAL_LABELS[k],winnerMean:am,otherMean:bm,diff:am!=null&&bm!=null?am-bm:null,effect:pooled&&pooled>0?(am-bm)/pooled:null,winnerN:a.length,otherN:b.length,coverage:Math.round((a.length+b.length)/Math.max(1,rows.length)*100)}}).sort((a,b)=>(Math.abs(b.effect??b.diff??-999)-Math.abs(a.effect??a.diff??-999)));
 const labels=ANALYSIS_LABEL_ORDER.map(label=>({label,count:rows.filter(x=>x.label.achievement===label).length})).filter(x=>x.count);
 return{dataset:ds,teacherCount:rows.length,winnerCount:winners.length,comparisonCount:others.length,labels,fields,analyzedAt:new Date().toISOString()}}
function teacherAnalysisHtml(){const a=teacherAnalysis();state.analysisMeta={datasetSchema:TEACHER_DATASET_SCHEMA,lastAnalyzedAt:a.analyzedAt,lastTeacherCount:a.teacherCount};save();const labelHtml=a.labels.map(x=>`<span class="badge">${esc(x.label)} ${x.count}</span>`).join(' ');const usable=a.fields.filter(x=>x.diff!=null);const rows=usable.map((x,i)=>`<tr><td>${i+1}</td><td>${esc(x.label)}</td><td>${x.winnerMean.toFixed(2)} (${x.winnerN})</td><td>${x.otherMean.toFixed(2)} (${x.otherN})</td><td class="${x.diff>=0?'analysis-positive':'analysis-negative'}">${x.diff>=0?'+':''}${x.diff.toFixed(2)}</td><td>${x.effect==null?'―':x.effect.toFixed(2)}</td><td>${x.coverage}%</td></tr>`).join('');return`<div class="dialog-head"><div><div class="eyebrow">Teacher Data Analyzer</div><h2>教師データ一次解析</h2></div><button type="button" class="icon-btn detail-top-close">×</button></div><section class="analysis-summary"><div><span>教師データ</span><strong>${a.teacherCount}頭</strong></div><div><span>重賞勝ち群</span><strong>${a.winnerCount}頭</strong></div><div><span>比較群</span><strong>${a.comparisonCount}頭</strong></div></section><p>${labelHtml}</p><p class="muted">重賞勝ち群と、実績ラベルが設定された非重賞勝ち群を比較しています。差は5点尺度の平均差、効果量は標準化差です。欠損値は項目ごとに除外します。</p>${usable.length?`<div class="analysis-table-wrap"><table class="analysis-table"><thead><tr><th>#</th><th>項目</th><th>重賞勝ち平均</th><th>比較群平均</th><th>平均との差</th><th>効果量</th><th>充足</th></tr></thead><tbody>${rows}</tbody></table></div>`:'<div class="empty"><h3>比較可能な項目がありません</h3><p>重賞勝ち群・比較群の双方に評価点と実績ラベルが必要です。</p></div>'}<h3>解釈上の注意</h3><div class="notes">この画面は初期の記述統計です。因果関係や将来確率を確定するものではありません。評価基準の統一、非重賞群の充実、性別・芝ダート等の層別化を行った後に2026 Season Modelの重みを決定します。</div>`}
