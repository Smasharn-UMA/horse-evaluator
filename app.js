...
(()=>{'use strict';
const K='horseEvaluator3',V='3.1.42',SCHEMA_VERSION=6,UI_K='horseEvaluator3_ui',PRE_IMPORT_K='horseEvaluator3_preImportBackup',PHOTO_DB='horseEvaluator3_photos',PHOTO_STORE='photos',VIDEO_DB='horseEvaluator3_videos',VIDEO_STORE='videos',$=id=>document.getElementById(id);let state;
...
// === 2026 Body Profile v1 ===
// 馬体解析は競走実績・血統・厩舎・生産育成・価格を参照しない。
// 写真評価・歩様評価・測尺だけを表示/派生計算に使用する。
const BODY_PROFILE_THRESHOLDS={standout:4.75,high:4.50,good:4.25,caution:4.00,bottleneck:3.50};
const BODY_PROFILE_GROUPS={
 hind:{label:'後躯',fields:['hindquarter','hindShape','hindStep','propulsion']},
 core:{label:'体幹',fields:['back','balance']},
 front:{label:'前躯',fields:['shoulder','chest','frontRange','stride']},
 motion:{label:'動作品質',fields:['rhythm','symmetry','flexibility','lightness']},
 growth:{label:'成長・骨格',fields:['bone','growth']}
};
...
function physicalScore(h,key){
 ...
 const x=scoreValue(ga[key]);return x;
}
...
function bodyProfile(h){
 ...
 if(movementValid){const min=Math.min(...movement.map(g=>g.value));linkage=min>=4.75?{grade:'S',label:'後躯→体幹→前躯すべて突出',key:'standout'}:min>=4.5?{grade:'A',label:'3領域すべて高水準',key:'high'}:min>=4.25?{grade:'B',label:'連動性は良好',key:'good'}:min>=4.0?{grade:'C',label:'一部に軽度注意',key:'caution'}:{grade:'D',label:'連動経路にボトルネック',key:'bottleneck'}}
...
function bodyProfileMini(h){const p=bodyProfile(h);if(p.validCount<3)return'';const weak=p.bottlenecks.length?`弱点 ${p.bottlenecks.length}`:'弱点なし';return`<div class="body-profile-mini"><b>身体</b><span>高水準 ${p.highCount}/5</span><span>突出 ${p.standoutCount}/5</span><span>${weak}</span></div>`}
function bodyProfileHtml(h){const p=bodyProfile(h);if(!p.validCount)return'';const rows=p.groups.map(g=>`<div class="body-domain body-band-${g.band.key}"><div><b>${esc(g.label)}</b><small>${g.count}/${g.total}項目</small></div><strong>${g.value==null?'―':g.value.toFixed(2)}</strong><span>${g.band.label}</span></div>`).join('');const bottleneck=p.bottlenecks.length?p.bottlenecks.map(g=>`${g.label} ${g.value.toFixed(2)}`).join('・'):'なし';const caution=p.cautions.length?p.cautions.map(g=>g.label).join('・'):'なし';const m=p.measurements||{};const measure=`${m.weight==null?'―':m.weight+'kg'} / ${m.height==null?'―':m.height+'cm'} / ${m.girth==null?'―':m.girth+'cm'} / ${m.cannon==null?'―':m.cannon+'cm'}`;let warning='';if(p.posteriorGrowth)warning='後躯＋成長・骨格の複合ボトルネックを検出。';else if(p.compound)warning='複数領域のボトルネックを検出。';else if(p.bottlenecks.length===1)warning='単独ボトルネックを検出。';return`<section class="body-profile-card"><div class="body-profile-head"><div><div class="eyebrow">Physical Analysis 2026</div><h3>身体能力プロファイル</h3><p>写真・歩様・測尺のみ。競走実績・血統・厩舎・生産育成・価格は参照しません。</p></div><div class="linkage-grade body-band-${p.linkage.key}"><strong>${p.linkage.grade}</strong><span>全身連動</span></div></div><div class="body-domain-grid">${rows}</div><div class="body-profile-summary"><div><span>突出領域</span><b>${p.standoutCount}/5</b></div><div><span>高水準領域</span><b>${p.highCount}/5</b></div><div><span>ボトルネック</span><b>${esc(bottleneck)}</b></div><div><span>補償力</span><b>${esc(p.compensation)}</b></div></div><div class="body-profile-notes"><p><b>全身連動 ${esc(p.linkage.grade)}</b>：${esc(p.linkage.label)}</p><p>軽度注意：${esc(caution)}${warning?` ／ <b>${esc(warning)}</b>`:''}</p><p><b>測尺 ${p.measurementCount}/4</b>：${esc(measure)} <small>※測尺は現段階では領域点へ機械加点せず、身体情報として併記します。</small></p></div><p class="body-profile-rule">2026暫定閾値：4.75以上＝突出、4.50以上＝高水準、4.25以上＝良好、4.00〜4.24＝軽度注意、3.50〜3.99＝ボトルネック、3.50未満＝強い弱点。教師データ増加時に再検証します。</p></section>`}
// === END 2026 Body Profile v1 ===
