...
(()=>{'use strict';
const K='horseEvaluator3',V='3.1.51',SCHEMA_VERSION=7,UI_K='horseEvaluator3_ui',PRE_IMPORT_K='horseEvaluator3_preImportBackup',PHOTO_DB='horseEvaluator3_photos',PHOTO_STORE='photos',VIDEO_DB='horseEvaluator3_videos',VIDEO_STORE='videos',$=id=>document.getElementById(id);let state;
...
async function blindRenderDialog(selectBlindId=''){
 ...
 $('blindExportPackage').onclick=exportBlindPackage;$('blindExportSecret').onclick=exportBlindSecret;$('blindPasteResults').onclick=()=>{const raw=prompt('horse-evaluator-blind-results-1 のJSONを貼り付けてください');if(!raw)return;try{const r=applyBlindResultsObject(JSON.parse(raw));blindRenderDialog();toast(`${r.applied}件を保存しました${r.unknown.length?' / 未一致 '+r.unknown.length+'件':''}`)}catch(e){alert('結果JSONを取り込めません。\n'+e.message)}}
}
