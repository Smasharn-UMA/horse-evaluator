...
(()=>{'use strict';
const K='horseEvaluator3',V='3.1.44',SCHEMA_VERSION=6,UI_K='horseEvaluator3_ui',PRE_IMPORT_K='horseEvaluator3_preImportBackup',PHOTO_DB='horseEvaluator3_photos',PHOTO_STORE='photos',VIDEO_DB='horseEvaluator3_videos',VIDEO_STORE='videos',$=id=>document.getElementById(id);let state;
...
function bodyProfileMini(h){const p=bodyProfile(h);if(p.validCount<3)return'';const weak=p.bottlenecks.length?`弱点 ${p.bottlenecks.length}`:'弱点なし';const weakClass=p.bottlenecks.length?'has-weak':'no-weak';return`<div class="body-profile-mini ${weakClass}"><div class="body-mini-title"><b>身体</b><span class="body-mini-linkage">連動 ${esc(p.linkage.grade)}</span></div><div class="body-mini-kpi"><span>高水準</span><strong>${p.highCount}/5</strong></div><div class="body-mini-kpi"><span>突出</span><strong>${p.standoutCount}/5</strong></div><div class="body-mini-weak">${weak}</div></div>`}
