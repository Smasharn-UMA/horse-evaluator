(()=>{'use strict';
const K='horseEvaluator3',V='3.1.5',PRE_IMPORT_K='horseEvaluator3_preImportBackup',$=id=>document.getElementById(id);let state=load();
...
function detectFormat(text){const x=cleanText(text);if(/募集馬名[\s\S]*性別\s*[\/／]\s*毛色[\s\S]*募集総額\s*[\/／]\s*一口出資額/.test(x))return'silk';if(/PEGASUS|ユニオン|募集時のPR|馬体重[：:\s]*[0-9.]+\s*kg/.test(x))return'union';return''}
