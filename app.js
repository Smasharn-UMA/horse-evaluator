(()=>{'use strict';
const K='horseEvaluator3',V='3.1.12',UI_K='horseEvaluator3_ui',PRE_IMPORT_K='horseEvaluator3_preImportBackup',$=id=>document.getElementById(id);let state;
...
function card(h){const m=h.measurements||{},bg=h.photoUrl?`style="background-image:url('${esc(h.photoUrl)}')"`:'';return`<article class="horse-card"><div class="photo" ${bg}>${h.photoUrl?'':'NO PHOTO'}</div><div class="body"><div class="topline"><div><span class="badge">${h.year} ${esc(h.club)}</span>${h.horseNo!=null?`<span class="badge">No.${h.horseNo}</span>`:''}${h.stableArea?`<span class="badge">${esc(h.stableArea)}</span>`:''}<h3>${esc(h.name)}</h3><div class="horse-meta">${esc(h.sex)||'性別未設定'}${h.coatColor?'・'+esc(h.coatColor):''}${h.breeder?' ／ '+esc(h.breeder):''}</div>${(()=>{const q=evaluationAverage(h.evaluation);return q.average==null?'':`<div class="horse-meta">評価平均 <b>${q.average.toFixed(2)}</b>（${q.count}項目）</div>`})()}</div><button class="favorite ${h.favorite?'on':''}" data-a="favorite" data-id="${h.id}">★</button></div><div class="pedigree">父：${esc(h.sire)||'―'}<br>母：${esc(h.dam)||'―'}<br>母父：${esc(h.broodmareSire)||'―'}<br>厩舎：${esc(h.trainer)||'―'}</div><div class="price-box"><b>総額 ${h.price==null?'―':money(h.price)+'万円'}</b><span>${h.shareCount==null?'募集口数 ―':money(h.shareCount)+'口'} ／ 1口 ${h.sharePrice==null?'―':money(h.sharePrice)+'円'}</span></div><div class="metrics">${metric('体重',m.weight,'kg')}${metric('体高',m.height,'cm')}${metric('胸囲',m.girth,'cm')}${metric('管囲',m.cannon,'cm')}</div><div class="card-actions"><button data-a="detail" data-id="${h.id}">詳細</button><button data-a="edit" data-id="${h.id}">編集</button><button data-a="delete" data-id="${h.id}" class="danger">削除</button></div><div class="updated">更新：${fmt(h.updatedAt)}</div></div></article>`}
function render(){refreshFilters();const a=list();E.dashboard.innerHTML=[['登録頭数',state.horses.length],['年度数',new Set(state.horses.map(h=>h.year)).size],['クラブ数',new Set(state.horses.map(h=>h.club)).size],['お気に入り',state.horses.filter(h=>h.favorite).length]].map(([l,v])=>`<div class="stat"><div class="num">${v}</div><div class="label">${l}</div></div>`).join('');E.resultCount.textContent=state.horses.length===a.length?`${a.length}頭`:`${a.length}頭 / 全${state.horses.length}頭`;E.horseList.innerHTML=a.map(card).join('');E.emptyState.classList.toggle('hidden',a.length!==0)}
