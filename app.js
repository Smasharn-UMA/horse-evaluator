(()=>{'use strict';
const K='horseEvaluator3',V='3.1.14',UI_K='horseEvaluator3_ui',PRE_IMPORT_K='horseEvaluator3_preImportBackup',$=id=>document.getElementById(id);let state;
...
function card(h){const m=h.measurements||{},bg=h.photoUrl?`style="background-image:url('${esc(h.photoUrl)}')"`:'',r=recommendation(h);return`<article class="horse-card"><div class="photo" ${bg}>${h.photoUrl?'':'NO PHOTO'}</div><div class="body"><div class="topline"><div><span class="badge">${h.year} ${esc(h.club)}</span>${h.horseNo!=null?`<span class="badge">No.${h.horseNo}</span>`:''}${h.stableArea?`<span class="badge">${esc(h.stableArea)}</span>`:''}<h3>${esc(h.name)}</h3><div class="horse-meta">${esc(h.sex)||'性別未設定'}${h.coatColor?'・'+esc(h.coatColor):''}${h.breeder?' ／ '+esc(h.breeder):''}</div>${(()=>{const q=evaluationAverage(h.evaluation);return q.average==null?'':`<div class="horse-meta">評価平均 <b>${q.average.toFixed(2)}</b>（${q.count}項目）</div>`})()}${r.score==null?'':`<div class="decision-strip"><div class="recommend-badge ${r.className}">${r.grade}</div><div><span>総合点</span><strong>${r.score.toFixed(1)}</strong></div><div><span>自信度</span><strong>${r.confidence}%</strong></div></div>`}</div><button class="favorite ${h.favorite?'on':''}" data-a="favorite" data-id="${h.id}">★</button></div><div class="pedigree">父：${esc(h.sire)||'―'}<br>母：${esc(h.dam)||'―'}<br>母父：${esc(h.broodmareSire)||'―'}<br>厩舎：${esc(h.trainer)||'―'}</div><div class="price-box"><b>総額 ${h.price==null?'―':money(h.price)+'万円'}</b><span>${h.shareCount==null?'募集口数 ―':money(h.shareCount)+'口'} ／ 1口 ${h.sharePrice==null?'―':money(h.sharePrice)+'円'}</span></div><div class="metrics">${metric('体重',m.weight,'kg')}${metric('体高',m.height,'cm')}${metric('胸囲',m.girth,'cm')}${metric('管囲',m.cannon,'cm')}</div><div class="card-actions"><button data-a="detail" data-id="${h.id}">詳細</button><button data-a="edit" data-id="${h.id}">編集</button><button data-a="delete" data-id="${h.id}" class="danger">削除</button></div><div class="updated">更新：${fmt(h.updatedAt)}</div></div></article>`}
...
function detail(id){const h=state.horses.find(x=>x.id===id),m=h.measurements||{},ev=normalizeEvaluation(h.evaluation),avg=evaluationAverage(ev),r=recommendation(h),d=(l,v)=>`<div class="detail-item"><span>${l}</span><b>${esc(v)||'―'}</b></div>`;const scores=EVAL_FIELDS.map(k=>d(EVAL_LABELS[k],ev.scores[k]==null?'':ev.scores[k]+' / 5')).join('');E.detailContent.innerHTML=`<div class="dialog-head"><div><div class="eyebrow">${h.year} ${esc(h.club)} ${h.horseNo!=null?'No.'+h.horseNo:''}</div><h2>${esc(h.name)}</h2></div></div>${h.photoUrl?`<div class="detail-photo" style="background-image:url('${esc(h.photoUrl)}')"></div>`:''}${r.score==null?`<div class="decision-panel empty-decision"><strong>評価未入力</strong><span>評価シートを入力すると推奨度・総合点・自信度を表示します。</span></div>`:`<div class="decision-panel"><div class="recommend-badge large ${r.className}">${r.grade}</div><div><span>推奨度</span><strong>${r.grade}</strong></div><div><span>総合点</span><strong>${r.score.toFixed(1)}<small> / 100</small></strong></div><div><span>自信度</span><strong>${r.confidence}<small>%</small></strong></div></div>`}<div class="detail-grid">${d('性別',h.sex)}${d('毛色',h.coatColor)}${d('生年月日',h.birthDate)}${d('父',h.sire)}${d('母',h.dam)}${d('母父',h.broodmareSire)}${d('所属',h.stableArea)}${d('厩舎',h.trainer)}${d('生産牧場',h.breeder)}${d('育成牧場',h.trainingFarm)}${d('在厩場所',h.currentLocation)}${d('クラス',h.horseClass)}${d('内部管理ID',h.internalId)}${d('募集総額',h.price==null?'':money(h.price)+'万円')}${d('募集口数',h.shareCount==null?'':money(h.shareCount)+'口')}${d('1口価格',h.sharePrice==null?'':money(h.sharePrice)+'円')}${d('馬体重',m.weight==null?'':m.weight+'kg')}${d('体高',m.height==null?'':m.height+'cm')}${d('胸囲',m.girth==null?'':m.girth+'cm')}${d('管囲',m.cannon==null?'':m.cannon+'cm')}</div>${h.sourceUrl?`<a class="source-link" href="${esc(h.sourceUrl)}" target="_blank" rel="noopener">募集馬ページを開く</a>`:''}${h.videoUrl?`<p><a href="${esc(h.videoUrl)}" target="_blank" rel="noopener">歩様動画を開く</a></p>`:''}<h3 class="detail-section-title">評価シート</h3><div class="evaluation-summary"><strong>${avg.average==null?'未評価':avg.average.toFixed(2)+' / 5'}</strong><span class="score-badge">${avg.count}項目入力</span>${(()=>{const q=weightedScore(h);return q.score==null?'':`<div class="detail-total-score">重み付き総合点 <b>${q.score.toFixed(1)} / 100</b>（充足 ${q.coverage}%）</div>`})()}</div><div class="detail-grid">${scores}${d('競走実績',ev.achievement)}${d('主戦場',ev.distanceCategory)}${d('体質リスク',ev.healthRisk==null?'':ev.healthRisk+' / 5')}</div><h3>評価コメント</h3><div class="notes">${esc(ev.comment)||'―'}</div><h3>募集時PR</h3><div class="notes">${esc(h.recruitmentPr)||'―'}</div><h3>メモ</h3><div class="notes">${esc(h.notes)||'―'}</div><h3>更新履歴</h3><div class="notes">${(h.changeLog||[]).slice().reverse().map(x=>fmt(x.at)+'　'+esc(x.action)).join('\n')}</div>`;E.detailDialog.showModal()}
