...
(()=>{'use strict';
const K='horseEvaluator3',V='3.1.53',SCHEMA_VERSION=7,UI_K='horseEvaluator3_ui',PRE_IMPORT_K='horseEvaluator3_preImportBackup',PHOTO_DB='horseEvaluator3_photos',PHOTO_STORE='photos',VIDEO_DB='horseEvaluator3_videos',VIDEO_STORE='videos',$=id=>document.getElementById(id);let state;
...
const BLIND_PROTOCOL_VERSION='blind-v1.0';
const BLIND_FEATURES=['hindquarter','hindShape','chest','shoulder','back','bone','balance','growth','frontRange','hindStep','propulsion','flexibility','stride','rhythm','symmetry','lightness'];
...
function latestCompletedBlindRun(h){const runs=blindRuns(h).filter(r=>r.status==='completed'&&r.evaluatedAt);return runs.sort((a,b)=>new Date(b.evaluatedAt)-new Date(a.evaluatedAt))[0]||null}
function blindScore(h,key){const r=latestCompletedBlindRun(h);return r?scoreValue(r.scores?.[key]):null}
function originalPhysicalScore(h,key){
...
function physicalScore(h,key){const r=latestCompletedBlindRun(h);if(r&&BLIND_FEATURES.includes(key))return scoreValue(r.scores?.[key]);return originalPhysicalScore(h,key)}
function blindSectionAverage(h,fields){const r=latestCompletedBlindRun(h);if(!r)return null;const vals=fields.map(k=>scoreValue(r.scores?.[k])).filter(v=>v!=null);return vals.length?{average:vals.reduce((a,b)=>a+b,0)/vals.length,count:vals.length,total:fields.length}:null}
function blindSummary(h){const r=latestCompletedBlindRun(h);if(!r)return null;return{run:r,body:blindSectionAverage(h,BLIND_FEATURES.slice(0,8)),gait:blindSectionAverage(h,BLIND_FEATURES.slice(8)),all:blindSectionAverage(h,BLIND_FEATURES)}}
function blindPrimaryMini(h){const b=blindSummary(h);if(!b)return'';const val=x=>x?`${x.average.toFixed(2)} <small>(${x.count}/${x.total})</small>`:'未評価';return`<div class="blind-primary-mini"><span class="badge blind-primary-badge">BLIND主評価</span><b>馬体 ${val(b.body)}</b><b>歩様 ${val(b.gait)}</b></div>`}
function blindDetailHtml(h){const b=blindSummary(h);if(!b)return'';const rows=BLIND_FEATURES.map(k=>{const neu=scoreValue(b.run.scores?.[k]),old=scoreValue(b.run.originalScores?.[k]);return`<div class="blind-score-row"><span>${esc(EVAL_LABELS[k]||k)}</span><strong>${neu==null?'未評価':neu+' / 5'}</strong><small>旧 ${old==null?'―':old+' / 5'}</small></div>`}).join('');return`<section class="blind-primary-detail"><div class="blind-primary-head"><div><span class="badge blind-primary-badge">現在の主評価</span><h3>ブラインド再評価</h3><p>${esc(b.run.blindId)} ／ ${b.run.evaluatedAt?fmt(b.run.evaluatedAt):''}</p></div><div><b>馬体 ${b.body?b.body.average.toFixed(2):'―'}</b><b>歩様 ${b.gait?b.gait.average.toFixed(2):'―'}</b></div></div><p class="muted">再評価値を身体評価・類似度計算で優先します。null項目は旧評価で補完せず「未評価」として扱います。</p><div class="blind-score-grid">${rows}</div>${b.run.comment?`<div class="notes blind-comment"><b>再評価コメント</b>\n${esc(b.run.comment)}</div>`:''}</section>`}
...
function uiScoreValue(v){const x=scoreValue(v);return x==null?null:Math.max(1,Math.min(5,Math.round(x)))}
...
function transferGaitScoresToEvaluation(sourceScores=null){
  ...
  keys.forEach(k=>{
    ...
    targetEl.value=String(uiScoreValue(value));
...
function applyGaitAiJson(){try{const x=extractJsonObject($('aiGaitJson').value);GAIT_AI_FIELDS.forEach(k=>{const v=x[k]??x.scores?.[k];if(v!=null){const el=$(GAIT_AI_IDS[k]);if(el)el.value=uiScoreValue(v)??''}});if(x.summary!=null)$('aiGaitSummary').value=t(x.summary);const count=transferGaitScoresToEvaluation(x);$('aiGaitJson').value='';toast(count?`AI歩様評価を反映し、評価シートへ${count}項目転記しました`:'AI歩様評価を反映しました')}catch(e){console.error(e);alert('AI歩様評価JSONの形式が正しくありません。')}}
...
function copyFullAiTemplate(){const x=fullAiTemplate();x.analysisProtocol={purpose:'1歳夏の募集時点における純粋な馬体・歩様評価',allowedInputs:['添付された募集時の馬体写真','添付された募集時の歩様動画','measurementsの測尺4項目'],prohibitedInputs:['父・母・母父などの血統','厩舎','生産牧場・育成牧場','募集価格','競走実績・クラス','他の募集馬との比較','募集馬群内順位'],scoreScale:'すべてのphotoAi・gaitAi・evaluation.scoresは1〜5の整数。1=低い、3=標準、5=非常に高い。100点尺度・小数点評価は使用しない。',rules:['horse内の血統・厩舎・生産情報は馬の識別情報として保持するだけで、身体評価の根拠に使用しない','同じ写真・動画・測尺なら、競走実績等を知っていても身体評価を変えない','photoAi.explosionはトモの爆発力として1〜5の整数で評価する','evaluation.scores.hindShapeはトモ形状・爆発力を1〜5の整数で評価する','evaluation.scoresのpedigree・trainer・farmは必ずnullのまま返す','適性（芝・ダート・距離）や募集馬群での順位は推測・記載しない','summary/commentには写真・歩様・測尺から直接観察できる根拠だけを書く','healthRiskは写真・歩様・測尺だけで明確に評価できない場合null。入力する場合も1〜5（1=低い、5=高い）とし、医学的診断や将来の故障を断定しない']};x.instruction='analysisProtocolを厳守し、写真・歩様動画・測尺のみを解析してください。このJSON構造とキーを維持し、評価値は必ず1〜5の整数で入力してください。小数は使用しないでください。pedigree・trainer・farmはnullのままにしてください。JSON以外の文章は返さないでください。';navigator.clipboard?.writeText(JSON.stringify(x,null,2)).then(()=>toast('統合AIテンプレート（固定解析仕様）をコピーしました')).catch(()=>alert(JSON.stringify(x,null,2)))}
function applyFullAiJson(){try{const x=extractJsonObject($('fullAiJson').value),h=x.horse||x.profile||{},editingId=t($('horseId').value),currentName=t($('name').value),jsonName=t(h.name);if(editingId&&jsonName&&currentName&&jsonName!==currentName&&!confirm(`JSONの馬名「${jsonName}」は編集中の「${currentName}」と一致しません。\nAI評価だけをこの馬へ反映しますか？`))return;if(!editingId){const fields=['name','club','horseNo','sex','coatColor','birthDate','sire','dam','broodmareSire','stableArea','trainer','breeder','trainingFarm','price','shareCount','sharePrice','sourceUrl'];fields.forEach(k=>{if(h[k]!=null&&$(k))$(k).value=h[k]})}Object.entries(x.measurements||{}).forEach(([k,v])=>{if(['weight','height','girth','cannon'].includes(k)&&v!=null)$(k).value=v});const pa=x.photoAi||{};PHOTO_AI_FIELDS.forEach(k=>{const v=pa[k]??pa.scores?.[k];if(v!=null)$(PHOTO_AI_IDS[k]).value=uiScoreValue(v)??''});if(pa.summary!=null)$('aiPhotoSummary').value=t(pa.summary);const ga=x.gaitAi||x.videoAi||{};GAIT_AI_FIELDS.forEach(k=>{const v=ga[k]??ga.scores?.[k];if(v!=null)$(GAIT_AI_IDS[k]).value=uiScoreValue(v)??''});if(ga.summary!=null)$('aiGaitSummary').value=t(ga.summary);const gaitTransferred=transferGaitScoresToEvaluation(ga);const ev=x.evaluation||{};EVAL_FIELDS.forEach(k=>{const v=ev[k]??ev.scores?.[k];if(v!=null)$(EVAL_IDS[k]).value=uiScoreValue(v)??''});if(ev.healthRisk!=null)$('healthRisk').value=scoreValue(ev.healthRisk)??'';if(ev.comment!=null)$('evaluationComment').value=t(ev.comment);$('fullAiJson').value='';toast(gaitTransferred?`ChatGPT統合JSONを反映し、歩様評価シートへ${gaitTransferred}項目転記しました`:'ChatGPT統合JSONを反映しました')}catch(e){console.error(e);alert('統合JSONの形式が正しくありません。')}}

function mainPhoto(h){const p=normalizePhotos(h);return p.photos.find(x=>x.id===p.mainPhotoId)?.src||p.photos[0]?.src||t(h.photoUrl)}
function base(){return{version:V,schemaVersion:SCHEMA_VERSION,app:'Horse Evaluator',modelSettings:normalizeModel(DEFAULT_MODEL),analysisMeta:{datasetSchema:TEACHER_DATASET_SCHEMA,lastAnalyzedAt:'',lastTeacherCount:0},blindReevaluationMeta:{protocolVersion:BLIND_PROTOCOL_VERSION,lastBatchId:'',lastCreatedAt:''},horses:[],updatedAt:new Date().toISOString()}}
...
function migrate(x){
  ...
  return {
    version:V,schemaVersion:SCHEMA_VERSION,sourceVersion:t(x?.version)||'unknown',app:'Horse Evaluator',modelSettings:normalizeModel(x?.modelSettings),analysisMeta:{datasetSchema:TEACHER_DATASET_SCHEMA,lastAnalyzedAt:t(x?.analysisMeta?.lastAnalyzedAt),lastTeacherCount:Number(x?.analysisMeta?.lastTeacherCount||0)},blindReevaluationMeta:{protocolVersion:t(x?.blindReevaluationMeta?.protocolVersion)||BLIND_PROTOCOL_VERSION,lastBatchId:t(x?.blindReevaluationMeta?.lastBatchId),lastCreatedAt:t(x?.blindReevaluationMeta?.lastCreatedAt)},updatedAt:now,
    horses:a.map((h,i)=>{
      ...
      return {
        ...
        videoUrl:t(pick(h.videoUrl,h.video,h.files?.video)),...normalizeVideos(h),favorite:Boolean(h.favorite),notes:t(pick(h.notes,h.memo)),management:{owned:Boolean(h.management?.owned??h.ownedHorse),comparisonExcluded:Boolean(h.management?.comparisonExcluded??h.comparisonExcluded),tags:Array.isArray(h.management?.tags)?h.management.tags.map(t).filter(Boolean):Array.isArray(h.tags)?h.tags.map(t).filter(Boolean):[]},teacherData:{enabled:Boolean(h.teacherData?.enabled??h.isTeacherData),rank:t(h.teacherData?.rank??h.teacherRank),registeredAt:t(h.teacherData?.registeredAt),finalRating:scoreValue(h.teacherData?.finalRating??h.finalRating),satisfaction:n(h.teacherData?.satisfaction??h.satisfaction),retrospective:t(h.teacherData?.retrospective??h.retrospective),comment:t(h.teacherData?.comment??h.teacherComment),gradedWinner:Boolean(h.teacherData?.gradedWinner||(t(h.evaluation?.achievement)==='重賞勝利')||(t(h.evaluation?.achievement)==='GⅠ勝利')),g1Winner:Boolean(h.teacherData?.g1Winner||(t(h.evaluation?.achievement)==='GⅠ勝利')),blindReevaluation:normalizeBlindReevaluation(h.teacherData?.blindReevaluation)},evaluation:normalizeEvaluation(h.evaluation),
...
function normalizeBlindReevaluation(x){
 const src=x&&typeof x==='object'?x:{};const runs=Array.isArray(src.runs)?src.runs:[];
 return{runs:runs.map(r=>({runId:t(r.runId)||uid(),batchId:t(r.batchId),blindId:t(r.blindId),protocolVersion:t(r.protocolVersion)||BLIND_PROTOCOL_VERSION,status:['queued','completed','adopted'].includes(t(r.status))?t(r.status):'queued',createdAt:t(r.createdAt),evaluatedAt:t(r.evaluatedAt),modelVersion:t(r.modelVersion),originalScores:Object.fromEntries(BLIND_FEATURES.map(k=>[k,scoreValue(r.originalScores?.[k])])),scores:Object.fromEntries(BLIND_FEATURES.map(k=>[k,scoreValue(r.scores?.[k])])),comment:t(r.comment)})).slice(-20)}
}
function blindRuns(h){return normalizeBlindReevaluation(h?.teacherData?.blindReevaluation).runs}
function latestBlindRun(h){const a=blindRuns(h);return a[a.length-1]||null}
function blindFullMedia(h){const p=normalizePhotos(h).photos||[],v=normalizeVideos(h).videos||[],m=h?.measurements||{};return{photo:p.length>0,gait:v.length>0,measurement:['weight','height','girth','cannon'].every(k=>m[k]!=null&&m[k]!==''),full:p.length>0&&v.length>0&&['weight','height','girth','cannon'].every(k=>m[k]!=null&&m[k]!=='')}}
function blindOriginalScores(h){return Object.fromEntries(BLIND_FEATURES.map(k=>[k,physicalScore(h,k)]))}
function blindCandidates(group='all'){
 return(state?.horses||[]).filter(h=>{if(!h.teacherData?.enabled||!blindFullMedia(h).full)return false;const a=resolvedTeacherAchievement(h)||'';if(group==='g1')return a==='GⅠ勝利';if(group==='graded')return a==='重賞勝利';if(group==='maiden')return a==='未勝利';if(group==='nonwinner')return !['GⅠ勝利','重賞勝利'].includes(a);return true})
}
function blindAverage(scores){const a=BLIND_FEATURES.map(k=>scoreValue(scores?.[k])).filter(v=>v!=null);return a.length?a.reduce((x,y)=>x+y,0)/a.length:null}
function blindCreateQueue(group,count){
 const candidates=blindCandidates(group).filter(h=>{const r=latestBlindRun(h);return !r||r.status==='adopted'});if(!candidates.length)throw new Error('条件に合う未キューの完全データ教師馬がありません。');
 const shuffled=[...candidates].sort(()=>Math.random()-.5),take=shuffled.slice(0,Math.max(1,Math.min(Number(count)||1,shuffled.length))),batchId='BLIND-'+new Date().toISOString().replace(/[-:.TZ]/g,'').slice(0,14);let idx=1;
 take.forEach(h=>{h.teacherData=h.teacherData||{};const br=normalizeBlindReevaluation(h.teacherData.blindReevaluation);br.runs.push({runId:uid(),batchId,blindId:`${batchId}-${String(idx++).padStart(3,'0')}`,protocolVersion:BLIND_PROTOCOL_VERSION,status:'queued',createdAt:new Date().toISOString(),evaluatedAt:'',modelVersion:'',originalScores:blindOriginalScores(h),scores:Object.fromEntries(BLIND_FEATURES.map(k=>[k,null])),comment:''});h.teacherData.blindReevaluation=br;h.changeLog=[...(h.changeLog||[]).slice(-49),{at:new Date().toISOString(),action:'ブラインド再評価キュー追加'}]});
 state.blindReevaluationMeta={protocolVersion:BLIND_PROTOCOL_VERSION,lastBatchId:batchId,lastCreatedAt:new Date().toISOString()};state=save();return{batchId,count:take.length}
}
function blindQueuedRuns(){const out=[];(state?.horses||[]).forEach(h=>blindRuns(h).forEach(r=>{if(['queued','completed'].includes(r.status))out.push({h,run:r})}));return out.sort((a,b)=>(a.run.batchId||'').localeCompare(b.run.batchId||'')||(a.run.blindId||'').localeCompare(b.run.blindId||''))}
function blindPayload(h,r){const m=h.measurements||{};return{schema:'horse-evaluator-blind-item-1',blindId:r.blindId,sex:h.sex||null,measurements:{weight:m.weight??null,height:m.height??null,girth:m.girth??null,cannon:m.cannon??null},protocol:{version:r.protocolVersion||BLIND_PROTOCOL_VERSION,purpose:'1歳夏の募集時点における純粋な馬体・歩様評価',allowedInputs:['募集時の馬体写真','募集時の歩様動画','測尺4項目','性別'],prohibitedInputs:['馬名','クラブ','父・母・母父','厩舎','生産牧場・育成牧場','募集価格','競走実績・クラス','実績ラベル','他馬との比較','募集馬群内順位'],scoreScale:'1-5 integer; 1=低い, 3=標準, 5=非常に高い',outputKeys:BLIND_FEATURES},instruction:'添付された募集時写真・歩様動画・測尺だけを使い、返却JSONのscoresを1〜5整数で埋めてください。識別情報や競走実績を推測・参照しないでください。JSON以外は返さないでください。',output:{schema:'horse-evaluator-blind-results-1',protocolVersion:r.protocolVersion||BLIND_PROTOCOL_VERSION,results:[{blindId:r.blindId,scores:Object.fromEntries(BLIND_FEATURES.map(k=>[k,null])),comment:''}]}}}
function copyBlindPrompt(h,r){const payload=blindPayload(h,r),text=JSON.stringify(payload,null,2);navigator.clipboard?.writeText(text).then(()=>toast(`${r.blindId} の匿名プロンプトをコピーしました`)).catch(()=>prompt('コピーしてください',text))}
function dataUrlToFile(dataUrl,name){const m=String(dataUrl||'').match(/^data:([^;,]+)?(;base64)?,(.*)$/);if(!m)throw new Error('写真データをファイル化できません。');const type=m[1]||'image/jpeg',raw=m[2]?atob(m[3]):decodeURIComponent(m[3]),u8=new Uint8Array(raw.length);for(let i=0;i<raw.length;i++)u8[i]=raw.charCodeAt(i);return new File([u8],name,{type})}
function extForMime(type,fallback){const map={'image/jpeg':'jpg','image/png':'png','image/webp':'webp','video/mp4':'mp4','video/quicktime':'mov','video/webm':'webm'};return map[type]||fallback}
async function blindMediaFiles(h,r){const ps=normalizePhotos(h),p=ps.photos.find(x=>x.id===ps.mainPhotoId)||ps.photos[0],v=normalizeVideos(h).videos[0];if(!p)throw new Error('募集時写真が登録されていません。');if(!v)throw new Error('歩様動画が登録されていません。');let photoFile;if(p.src&&String(p.src).startsWith('data:')){const mime=(String(p.src).match(/^data:([^;,]+)/)||[])[1]||'image/jpeg';photoFile=dataUrlToFile(p.src,`${r.blindId}-photo.${extForMime(mime,'jpg')}`)}else if(p.dbKey){const data=await photoDbGet(p.dbKey);if(!data)throw new Error('IndexedDBから写真を取得できません。');const mime=(String(data).match(/^data:([^;,]+)/)||[])[1]||'image/jpeg';photoFile=dataUrlToFile(data,`${r.blindId}-photo.${extForMime(mime,'jpg')}`)}else throw new Error('写真の保存データが見つかりません。');
 let videoBlob=null;if(v.dbKey)videoBlob=await videoDbGet(v.dbKey);if(!videoBlob&&v._blob)videoBlob=v._blob;if(!videoBlob)throw new Error('IndexedDBから歩様動画を取得できません。');const videoType=videoBlob.type||'video/mp4',videoFile=new File([videoBlob],`${r.blindId}-gait.${extForMime(videoType,'mp4')}`,{type:videoType});const payload=blindPayload(h,r),jsonFile=new File([JSON.stringify(payload,null,2)],`${r.blindId}-request.json`,{type:'application/json'});return{photoFile,videoFile,jsonFile,payload}}
function downloadFile(file){const u=URL.createObjectURL(file),a=document.createElement('a');a.href=u;a.download=file.name;a.click();setTimeout(()=>URL.revokeObjectURL(u),1500)}
async function shareBlindBundle(h,r){try{const b=await blindMediaFiles(h,r),files=[b.photoFile,b.videoFile,b.jsonFile];if(navigator.share&&(!navigator.canShare||navigator.canShare({files}))){await navigator.share({title:`Horse Evaluator ${r.blindId}`,text:'募集時写真・歩様動画・匿名再評価JSONです。JSONの指示に従って完全ブラインド再評価してください。',files});toast(`${r.blindId} の再評価素材を共有しました`);return}files.forEach(downloadFile);toast('共有非対応のため写真・動画・JSONを自動保存しました')}catch(e){if(e?.name==='AbortError')return;console.error(e);alert('再評価素材を準備できません。\n'+e.message)}}
function blindPackageObject(){const q=blindQueuedRuns();return{schema:'horse-evaluator-blind-reevaluation-package-2',createdAt:new Date().toISOString(),protocolVersion:BLIND_PROTOCOL_VERSION,horseCount:q.length,horses:q.map(({h,run})=>{const x=blindPayload(h,run);return{blindId:x.blindId,sex:x.sex,measurements:x.measurements,mediaRequired:{photo:true,gait:true},protocol:x.protocol}})}}
function blindSecretMapObject(){const q=blindQueuedRuns();return{schema:'horse-evaluator-blind-secret-map-2',createdAt:new Date().toISOString(),warning:'AI再評価時にはこのファイルを渡さないでください',mapping:q.map(({h,run})=>({blindId:run.blindId,horseId:h.id,name:h.name,achievement:resolvedTeacherAchievement(h)||null,originalScores:run.originalScores}))}}
function exportBlindPackage(){const x=blindPackageObject();if(!x.horseCount){alert('再評価キューがありません。');return}downloadJson(x,`horse-evaluator-blind-package-${new Date().toISOString().slice(0,10)}-${x.horseCount}horses.json`)}
function exportBlindSecret(){const x=blindSecretMapObject();if(!x.mapping.length){alert('再評価キューがありません。');return}downloadJson(x,`horse-evaluator-blind-SECRET-map-${new Date().toISOString().slice(0,10)}.json`)}
function applyBlindResultsObject(x){const list=Array.isArray(x)?x:(x?.results||[]);if(!Array.isArray(list))throw new Error('results配列がありません。');let applied=0,unknown=[];list.forEach(result=>{const id=t(result?.blindId);let found=null;(state.horses||[]).some(h=>{const br=normalizeBlindReevaluation(h.teacherData?.blindReevaluation),i=br.runs.findIndex(r=>r.blindId===id);if(i>=0){found={h,br,i};return true}return false});if(!found){if(id)unknown.push(id);return}const scores=result.scores||result.evaluation?.scores||{};const normalized=Object.fromEntries(BLIND_FEATURES.map(k=>[k,scoreValue(scores[k])]));const valid=Object.values(normalized).filter(v=>v!=null).length;if(!valid)return;const r=found.br.runs[found.i];r.scores=normalized;r.comment=t(result.comment||result.evaluation?.comment);r.modelVersion=t(result.modelVersion||x?.modelVersion);r.evaluatedAt=new Date().toISOString();r.status='completed';found.h.teacherData.blindReevaluation=found.br;found.h.changeLog=[...(found.h.changeLog||[]).slice(-49),{at:r.evaluatedAt,action:'ブラインド再評価結果を保存'}];applied++});state=save();return{applied,unknown}}
function blindDiffHtml(run){if(run.status!=='completed')return'<span class="badge">未評価</span>';const old=blindAverage(run.originalScores),neu=blindAverage(run.scores),d=(old!=null&&neu!=null)?neu-old:null;return`<span class="badge">旧 ${old==null?'―':old.toFixed(2)}</span> <span class="badge">新 ${neu==null?'―':neu.toFixed(2)}</span> <span class="badge ${d!=null&&d<0?'blind-diff-down':''}">差 ${d==null?'―':(d>0?'+':'')+d.toFixed(2)}</span>`}
async function hydrateBlindMedia(h){
 const photos=Array.isArray(h?.photos)?h.photos:[],videos=Array.isArray(h?.videos)?h.videos:[];let photoOk=false,gaitOk=false;
 for(const p of photos){if(!p.src&&p.dbKey){try{p.src=await photoDbGet(p.dbKey)}catch(e){console.warn('ブラインド写真読込失敗',p.dbKey,e)}}if(p.src)photoOk=true}
 for(const v of videos){if(!v.src&&v.dbKey){try{const blob=await videoDbGet(v.dbKey);if(blob){v.src=URL.createObjectURL(blob);v._blindObjectUrl=true}}catch(e){console.warn('ブラインド動画読込失敗',v.dbKey,e)}}if(videoHref(v))gaitOk=true}
 return{photo:photoOk,gait:gaitOk}
}
function blindMediaHtml(h,mediaStatus={}){const p=normalizePhotos(h),img=p.photos.find(x=>x.id===p.mainPhotoId)||p.photos[0],v=normalizeVideos(h).videos[0],m=h.measurements||{};return`<section class="blind-current-media"><div class="blind-anon-head"><div><div class="eyebrow">完全匿名表示</div><h3 id="blindCurrentName">選択してください</h3><p id="blindCurrentId" class="muted"></p></div></div>${img?.src?`<img class="blind-photo" src="${esc(img.src)}" alt="匿名募集時写真">`:'<div class="photo-empty">写真をIndexedDBから取得できません</div>'}${v&&videoHref(v)?`<video class="blind-video" controls playsinline preload="metadata" src="${esc(videoHref(v))}"></video>`:'<div class="photo-empty">歩様動画をIndexedDBから取得できません</div>'}<p class="muted">メディア読込：写真 ${mediaStatus.photo?'✓':'×'} ／ 歩様 ${mediaStatus.gait?'✓':'×'}。匿名JSONのコピーだけではChatGPTにメディアは添付されません。表示された写真・動画を別途添付してください。</p><div class="detail-grid">${['weight','height','girth','cannon'].map((k,i)=>`<div class="detail-item"><span>${['馬体重','体高','胸囲','管囲'][i]}</span><b>${m[k]==null?'―':esc(m[k])+(['kg','cm','cm','cm'][i])}</b></div>`).join('')}<div class="detail-item"><span>性別</span><b>${esc(h.sex)||'―'}</b></div></div></section>`}
async function blindRenderDialog(selectBlindId=''){
 const box=$('blindReevalContent');if(!box)return;const q=blindQueuedRuns(),complete=blindCandidates('all').length,g1=blindCandidates('g1').length,done=q.filter(x=>x.run.status==='completed').length;const selected=q.find(x=>x.run.blindId===selectBlindId)||q[0];const mediaStatus=selected?await hydrateBlindMedia(selected.h):{};
 box.innerHTML=`<section class="blind-controls"><div class="analysis-summary"><div><span>完全データ教師馬</span><strong>${complete}頭</strong></div><div><span>うちGⅠ</span><strong>${g1}頭</strong></div><div><span>キュー</span><strong>${q.length}件</strong></div><div><span>再評価済み</span><strong>${done}件</strong></div></div><div class="form-grid four"><label>抽出群<select id="blindGroup"><option value="g1">GⅠ勝利</option><option value="graded">重賞勝利</option><option value="maiden">未勝利</option><option value="nonwinner">非重賞群</option><option value="all">完全データ全体</option></select></label><label>無作為抽出数<input id="blindCount" type="number" min="1" value="${Math.min(17,Math.max(1,g1||complete))}"></label><label>プロトコル<input value="${BLIND_PROTOCOL_VERSION}" disabled></label><button type="button" id="blindCreateQueue" class="primary">キュー作成</button></div><div class="dialog-actions blind-actions"><button id="blindExportPackage" type="button" class="secondary">匿名パッケージJSON</button><button id="blindExportSecret" type="button" class="secondary">SECRET照合表</button><button id="blindPasteResults" type="button" class="secondary">結果JSONを貼付</button></div><p class="muted">完全データ＝募集時写真＋歩様動画＋測尺4項目。キュー作成時点の旧16項目評価を固定保存し、後から上書きしません。</p></section>${q.length?`<div class="blind-workspace"><div class="blind-queue"><h3>再評価キュー</h3>${q.map(({h,run})=>`<button type="button" class="blind-queue-row ${selected?.run.blindId===run.blindId?'active':''}" data-blind-open="${esc(run.blindId)}"><span><b>${esc(h.name)||'名称未設定'}</b><small>${esc(run.blindId)} ／ ${run.status==='completed'?'再評価済み':'待機中'} ／ 写真● 歩様● 測尺●</small></span><span>${blindDiffHtml(run)}</span></button>`).join('')}</div><div class="blind-current">${selected?blindMediaHtml(selected.h,mediaStatus):''}${selected?`<div class="blind-current-actions"><button type="button" class="primary" id="blindShareBundle">再評価素材をまとめて共有</button><button type="button" class="secondary" id="blindCopyPrompt">匿名JSONのみコピー</button><button type="button" class="secondary" id="blindPasteOne">この馬の結果JSONを貼付</button></div><p class="muted">「再評価素材をまとめて共有」はIndexedDBから写真・歩様動画を自動取得し、匿名JSONと3ファイルを共有します。元ファイルを探し直す必要はありません。共有先には馬名・血統・厩舎・生産・価格・競走実績を含めません。</p>`:''}</div></div>`:'<div class="empty"><h3>再評価キューはありません</h3><p>抽出群と頭数を選んでキューを作成してください。</p></div>'}`;
 if(selected){$('blindCurrentName').textContent=selected.h.name||'名称未設定';$('blindCurrentId').textContent=selected.run.blindId;$('blindShareBundle').disabled=!(mediaStatus.photo&&mediaStatus.gait);$('blindShareBundle').title=(mediaStatus.photo&&mediaStatus.gait)?'保存済み写真・歩様動画・匿名JSONをまとめて共有します':'写真・歩様動画の読込完了後に使用できます';$('blindShareBundle').onclick=()=>shareBlindBundle(selected.h,selected.run);$('blindCopyPrompt').disabled=!(mediaStatus.photo&&mediaStatus.gait);$('blindCopyPrompt').onclick=()=>copyBlindPrompt(selected.h,selected.run);$('blindPasteOne').onclick=()=>{const raw=prompt(`${selected.run.blindId} の結果JSONを貼り付けてください`);if(!raw)return;try{const x=extractJsonObject(raw),r=applyBlindResultsObject(x.results?x:{results:[x]});blindRenderDialog(selected.run.blindId);toast(`${r.applied}件を保存しました`)}catch(e){alert('結果JSONを取り込めません。\n'+e.message)}}}
 box.querySelectorAll('[data-blind-open]').forEach(b=>b.onclick=()=>blindRenderDialog(b.dataset.blindOpen));
 $('blindCreateQueue').onclick=()=>{try{const r=blindCreateQueue($('blindGroup').value,$('blindCount').value);blindRenderDialog();toast(`${r.count}頭をブラインド再評価キューへ追加しました`)}catch(e){alert(e.message)}};
 $('blindExportPackage').onclick=exportBlindPackage;$('blindExportSecret').onclick=exportBlindSecret;$('blindPasteResults').onclick=()=>{const raw=prompt('horse-evaluator-blind-results-1 のJSONを貼り付けてください');if(!raw)return;try{const r=applyBlindResultsObject(extractJsonObject(raw));blindRenderDialog();toast(`${r.applied}件を保存しました${r.unknown.length?' / 未一致 '+r.unknown.length+'件':''}`)}catch(e){alert('結果JSONを取り込めません。\n'+e.message)}}
}
function openBlindReevaluation(){$('blindReevalDialog').showModal();blindRenderDialog();requestAnimationFrame(()=>$('blindReevalDialog').scrollTop=0)}
...
$('exportBtn').onclick=exportJ;$('exportTeacherBtn').onclick=exportTeacherDataset;$('teacherAnalysisBtn').onclick=openTeacherAnalysis;$('blindReevalBtn').onclick=openBlindReevaluation;$('blindCloseBtn').onclick=()=>$('blindReevalDialog').close();$('importInput').addEventListener('change',e=>{const f=e.target.files&&e.target.files[0];if(E.restoreStatus){E.restoreStatus.textContent=f?`「${f.name}」を選択しました。復元を開始します…`:'ファイルが選択されませんでした。';E.restoreStatus.className='muted'}if(f){setTimeout(()=>importJ(f,true),0)}e.target.value='';});
