(()=>{'use strict';
const K='horseEvaluator3',V='3.1.13',UI_K='horseEvaluator3_ui',PRE_IMPORT_K='horseEvaluator3_preImportBackup',$=id=>document.getElementById(id);let state;
...
const DEFAULT_MODEL={name:'標準モデル',weights:{gait:30,body:25,growth:15,measurement:10,pedigree:10,connections:10}};
...
function initEvaluationControls(){EVAL_FIELDS.forEach(k=>{const el=$(EVAL_IDS[k]);if(el)el.innerHTML=scoreOptions()})}

function normalizeModel(m={}){const w=m.weights||{};return{name:t(m.name)||DEFAULT_MODEL.name,weights:{gait:Number(w.gait??30),body:Number(w.body??25),growth:Number(w.growth??15),measurement:Number(w.measurement??10),pedigree:Number(w.pedigree??10),connections:Number(w.connections??10)}}}
  ...
  return {
    ...
    horses:a.map((h,i)=>{
      ...
      return {
        ...
        videoUrl:t(pick(h.videoUrl,h.video,h.files?.video)),favorite:Boolean(h.favorite),notes:t(pick(h.notes,h.memo)),evaluation:normalizeEvaluation(h.evaluation),
...
function openNew(){E.horseForm.reset();EVAL_FIELDS.forEach(k=>{const el=$(EVAL_IDS[k]);if(el)el.value=''});$('horseId').value='';$('year').value=new Date().getFullYear();$('dialogTitle').textContent='新規登録';setImportStatus('');if(E.pageText)E.pageText.value='';E.horseDialog.showModal()}
...
function openModelSettings(){const m=normalizeModel(state.modelSettings);$('modelName').value=m.name;Object.entries(modelInputs()).forEach(([k,el])=>el.value=m.weights[k]);updateWeightTotal();$('modelDialog').showModal()}
function setDefaultWeights(){const m=normalizeModel(DEFAULT_MODEL);$('modelName').value=m.name;Object.entries(modelInputs()).forEach(([k,el])=>el.value=m.weights[k]);updateWeightTotal()}
function renderModelSummary(){const m=normalizeModel(state.modelSettings),w=m.weights;$('modelSummary').textContent=`${m.name}：歩様${w.gait}%・馬体${w.body}%・成長${w.growth}%・測尺${w.measurement}%・配合${w.pedigree}%・厩舎牧場${w.connections}%`}
