...
(()=>{'use strict';
const K='horseEvaluator3',V='3.1.31',UI_K='horseEvaluator3_ui',PRE_IMPORT_K='horseEvaluator3_preImportBackup',PHOTO_DB='horseEvaluator3_photos',PHOTO_STORE='photos',VIDEO_DB='horseEvaluator3_videos',VIDEO_STORE='videos',$=id=>document.getElementById(id);let state;
  ...
  return {
    ...
    horses:a.map((h,i)=>{
      ...
      return {
        ...
        videoUrl:t(pick(h.videoUrl,h.video,h.files?.video)),...normalizeVideos(h),favorite:Boolean(h.favorite),notes:t(pick(h.notes,h.memo)),management:{owned:Boolean(h.management?.owned??h.ownedHorse),comparisonExcluded:Boolean(h.management?.comparisonExcluded??h.comparisonExcluded),tags:Array.isArray(h.management?.tags)?h.management.tags.map(t).filter(Boolean):Array.isArray(h.tags)?h.tags.map(t).filter(Boolean):[]},teacherData:{enabled:Boolean(h.teacherData?.enabled??h.isTeacherData),rank:t(h.teacherData?.rank??h.teacherRank),registeredAt:t(h.teacherData?.registeredAt),finalRating:scoreValue(h.teacherData?.finalRating??h.finalRating),satisfaction:n(h.teacherData?.satisfaction??h.satisfaction),retrospective:t(h.teacherData?.retrospective??h.retrospective)},evaluation:normalizeEvaluation(h.evaluation),
