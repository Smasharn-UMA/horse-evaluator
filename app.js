(()=>{'use strict';
const K='horseEvaluator3',V='3.1.6',PRE_IMPORT_K='horseEvaluator3_preImportBackup',$=id=>document.getElementById(id);let state=load();
...
function parseUnion(text,url){
 ...
 const candidate=[...x.matchAll(/([^\s|｜<>]{1,40}の20\d{2})/g)].map(m=>m[1]).find(v=>!/募集|年齢/.test(v));
 const name=named||candidate||top.replace(/^\s*(?:PEGASUS\s*)?[0-9]+[\.．\s]+/i,'').trim()||(dam&&birthYear?`${dam}の${birthYear}`:'');
 ...
 const recruitmentPr=first(x,/募集時のPR[\s\S]*?(?:ポイント\s*)?([\s\S]*?)(?=生産者からのPR|主な兄姉・近親|近況|$)/)||labelValue(['PR','募集時PR']);
