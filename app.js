...
(()=>{'use strict';
const K='horseEvaluator3',V='3.1.35',SCHEMA_VERSION=3,UI_K='horseEvaluator3_ui',PRE_IMPORT_K='horseEvaluator3_preImportBackup',PHOTO_DB='horseEvaluator3_photos',PHOTO_STORE='photos',VIDEO_DB='horseEvaluator3_videos',VIDEO_STORE='videos',$=id=>document.getElementById(id);let state;
...
function parseCarrotActive(text,url=''){
 ...
 const data={year,club:'キャロット',horseNo:null,name,englishName,nameOrigin,sex:sexValue,coatColor,birthDate,sire,dam,broodmareSire,pedigreeCross,stableArea,trainer:trainerRaw.replace(/\s+/g,''),breeder,trainingFarm:'',currentLocation:'',horseClass,price,shareCount,sharePrice,recruitmentPr:'',sourceUrl:url,notes,currentStatus:{surface,className,recordText,wins,seconds,thirds,others,starts},measurements:{weight:null,height:null,girth:null,cannon:null}};
 data.internalId=internalId(data.club,data.year,data.horseNo,'');return data;
}
...
function applyImportedData(data){
  pendingImportedExtras={englishName:t(data.englishName),nameOrigin:t(data.nameOrigin),pedigreeCross:t(data.pedigreeCross),currentStatus:data.currentStatus?{...data.currentStatus}:null};
