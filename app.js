...
function normalizeAiAnalysis(x={}){return{date:t(x.date||x.analysisDate),inputs:{photo:Boolean(x.inputs?.photo),gait:Boolean(x.inputs?.gait),measurements:Boolean(x.inputs?.measurements)},measurementLabel:t(x.measurementLabel)}}
...
function base(){return{version:V,schemaVersion:DATA_SCHEMA,app:'Horse Evaluator',modelSettings:normalizeModel(DEFAULT_MODEL),horses:[],updatedAt:new Date().toISOString()}}
...
function save(nextState=state){const candidate={...nextState,version:V,schemaVersion:DATA_SCHEMA,updatedAt:new Date().toISOString()},stored=persistableState(candidate),nextJson=JSON.stringify(stored),previousJson=localStorage.getItem(K);try{localStorage.removeItem(PRE_IMPORT_K);localStorage.removeItem(K);localStorage.setItem(K,nextJson);const check=localStorage.getItem(K);if(check!==nextJson)throw new Error('保存後のデータ確認に失敗しました。');return candidate}catch(error){try{localStorage.removeItem(K);if(previousJson!==null)localStorage.setItem(K,previousJson)}catch(rollbackError){console.error('保存データの復旧にも失敗しました',rollbackError)}const quota=error?.name==='QuotaExceededError'||/quota/i.test(error?.message||'');throw new Error(quota?'ブラウザの基本データ保存容量が不足しています。JSONバックアップ後に不要なデータを整理してください。':(error?.message||'保存処理に失敗しました。'))}}
...
function migrate(x){
  ...
  return {
    version:V,schemaVersion:DATA_SCHEMA,sourceVersion:t(x?.version)||'unknown',app:'Horse Evaluator',modelSettings:normalizeModel(x?.modelSettings),updatedAt:now,
    horses:a.map((h,i)=>{
      ...
      return {
        ...
        videoUrl:t(pick(h.videoUrl,h.video,h.files?.video)),...normalizeVideos(h),aiAnalysis:normalizeAiAnalysis(h.aiAnalysis||h.analysis),favorite:Boolean(h.favorite),notes:t(pick(h.notes,h.memo)),evaluation:normalizeEvaluation(h.evaluation),
...
async function exportJ(){await hydrateStatePhotos(state);const backup=persistableState({...state,version:V,schemaVersion:DATA_SCHEMA,backupPolicy:{photos:'metadata-only; device files remain in IndexedDB',videos:'not embedded; device files remain in IndexedDB'}});const b=new Blob([JSON.stringify({...backup,exportedAt:new Date().toISOString()},null,2)],{type:'application/json'}),u=URL.createObjectURL(b),a=document.createElement('a');a.href=u;a.download=`horse-evaluator-backup-${new Date().toISOString().slice(0,10)}.json`;a.click();URL.revokeObjectURL(u);toast('バックアップを書き出しました')}
