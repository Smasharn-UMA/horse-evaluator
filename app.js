(()=>{'use strict';
const K='horseEvaluator3',V='3.1.24',UI_K='horseEvaluator3_ui',PRE_IMPORT_K='horseEvaluator3_preImportBackup',$=id=>document.getElementById(id);let state;
...
function renderVideoEditor(){const box=$('videoEditorList');if(!box)return;$('videoCount').textContent=`${editVideos.length}件`;box.innerHTML=editVideos.length?editVideos.map((v,i)=>`<div class="video-edit-row"><div><b>動画${i+1}</b><a href="${esc(v.url)}" target="_blank" rel="noopener">開く</a><small>${esc(v.url)}</small></div><button type="button" data-video-delete="${v.id}" class="danger">削除</button></div>`).join(''):'<div class="photo-empty">動画URLはまだ登録されていません。</div>'}
function gaitAiHtml(h){const vm=normalizeVideos(h),ai=vm.gaitAi,rows=GAIT_AI_FIELDS.filter(k=>ai.scores[k]!=null).map(k=>`<div><span>${GAIT_AI_LABELS[k]}</span><b>${stars(ai.scores[k])} ${ai.scores[k]}/5</b></div>`).join('');if(!vm.videos.length&&!vm.gaitComment&&!rows&&!ai.summary)return'';return`<section class="photo-ai-detail"><h3>歩様動画・AI歩様評価</h3>${vm.videos.length?`<div class="video-links">${vm.videos.map((v,i)=>`<a href="${esc(v.url)}" target="_blank" rel="noopener">動画${i+1}を開く</a>`).join('')}</div>`:''}${vm.gaitComment?`<div class="notes"><b>歩様コメント</b>
...
function applyGaitAiJson(){try{const x=extractJsonObject($('aiGaitJson').value);GAIT_AI_FIELDS.forEach(k=>{if(x[k]!=null||x.scores?.[k]!=null)$(GAIT_AI_IDS[k]).value=scoreValue(x[k]??x.scores[k])??''});if(x.summary!=null)$('aiGaitSummary').value=t(x.summary);toast('AI歩様評価を反映しました')}catch{alert('AI歩様評価JSONの形式が正しくありません。')}}
function applyGaitToEvaluation(){GAIT_AI_FIELDS.filter(k=>k!=='overall').forEach(k=>{const val=$(GAIT_AI_IDS[k]).value,ev=$(EVAL_IDS[k]);if(val&&ev)ev.value=val});toast('歩様評価シートへ転記しました')}
...
function applyFullAiJson(){try{const x=extractJsonObject($('fullAiJson').value),h=x.horse||x.profile||{};const fields=['name','club','horseNo','sex','coatColor','birthDate','sire','dam','broodmareSire','stableArea','trainer','breeder','trainingFarm','price','shareCount','sharePrice','sourceUrl'];fields.forEach(k=>{if(h[k]!=null&&$(k))$(k).value=h[k]});Object.entries(x.measurements||{}).forEach(([k,v])=>{if(['weight','height','girth','cannon'].includes(k)&&v!=null)$(k).value=v});const pa=x.photoAi||{};PHOTO_AI_FIELDS.forEach(k=>{const v=pa[k]??pa.scores?.[k];if(v!=null)$(PHOTO_AI_IDS[k]).value=scoreValue(v)??''});if(pa.summary!=null)$('aiPhotoSummary').value=t(pa.summary);const ga=x.gaitAi||x.videoAi||{};GAIT_AI_FIELDS.forEach(k=>{const v=ga[k]??ga.scores?.[k];if(v!=null)$(GAIT_AI_IDS[k]).value=scoreValue(v)??''});if(ga.summary!=null)$('aiGaitSummary').value=t(ga.summary);const ev=x.evaluation||{};EVAL_FIELDS.forEach(k=>{const v=ev[k]??ev.scores?.[k];if(v!=null)$(EVAL_IDS[k]).value=scoreValue(v)??''});if(ev.healthRisk!=null)$('healthRisk').value=scoreValue(ev.healthRisk)??'';if(ev.comment!=null)$('evaluationComment').value=t(ev.comment);toast('ChatGPT統合JSONを反映しました')}catch(e){console.error(e);alert('統合JSONの形式が正しくありません。')}}
...
function applyPhotoAiJson(){try{const x=extractJsonObject($('aiPhotoJson').value);PHOTO_AI_FIELDS.forEach(k=>{if(x[k]!=null||x.scores?.[k]!=null)$(PHOTO_AI_IDS[k]).value=scoreValue(x[k]??x.scores[k])??''});if(x.summary!=null)$('aiPhotoSummary').value=t(x.summary);toast('AI写真評価を反映しました')}catch(e){console.error(e);alert('AI写真評価を反映できませんでした。\n'+(e.message||'入力内容を確認してください。'))}}
