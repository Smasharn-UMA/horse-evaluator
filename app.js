(()=>{'use strict';
const K='horseEvaluator3',V='3.1.27',UI_K='horseEvaluator3_ui',PRE_IMPORT_K='horseEvaluator3_preImportBackup',PHOTO_DB='horseEvaluator3_photos',PHOTO_STORE='photos',$=id=>document.getElementById(id);let state;
...
function applyGaitAiJson(){try{const x=extractJsonObject($('aiGaitJson').value);GAIT_AI_FIELDS.forEach(k=>{if(x[k]!=null||x.scores?.[k]!=null)$(GAIT_AI_IDS[k]).value=scoreValue(x[k]??x.scores[k])??''});if(x.summary!=null)$('aiGaitSummary').value=t(x.summary);$('aiGaitJson').value='';toast('AI歩様評価を反映しました')}catch{alert('AI歩様評価JSONの形式が正しくありません。')}}
...
function applyFullAiJson(){try{const x=extractJsonObject($('fullAiJson').value),h=x.horse||x.profile||{},editingId=t($('horseId').value),currentName=t($('name').value),jsonName=t(h.name);if(editingId&&jsonName&&currentName&&jsonName!==currentName&&!confirm(`JSONの馬名「${jsonName}」は編集中の「${currentName}」と一致しません。\nAI評価だけをこの馬へ反映しますか？`))return;if(!editingId){const fields=['name','club','horseNo','sex','coatColor','birthDate','sire','dam','broodmareSire','stableArea','trainer','breeder','trainingFarm','price','shareCount','sharePrice','sourceUrl'];fields.forEach(k=>{if(h[k]!=null&&$(k))$(k).value=h[k]})}Object.entries(x.measurements||{}).forEach(([k,v])=>{if(['weight','height','girth','cannon'].includes(k)&&v!=null)$(k).value=v});const pa=x.photoAi||{};PHOTO_AI_FIELDS.forEach(k=>{const v=pa[k]??pa.scores?.[k];if(v!=null)$(PHOTO_AI_IDS[k]).value=scoreValue(v)??''});if(pa.summary!=null)$('aiPhotoSummary').value=t(pa.summary);const ga=x.gaitAi||x.videoAi||{};GAIT_AI_FIELDS.forEach(k=>{const v=ga[k]??ga.scores?.[k];if(v!=null)$(GAIT_AI_IDS[k]).value=scoreValue(v)??''});if(ga.summary!=null)$('aiGaitSummary').value=t(ga.summary);const ev=x.evaluation||{};EVAL_FIELDS.forEach(k=>{const v=ev[k]??ev.scores?.[k];if(v!=null)$(EVAL_IDS[k]).value=scoreValue(v)??''});if(ev.healthRisk!=null)$('healthRisk').value=scoreValue(ev.healthRisk)??'';if(ev.comment!=null)$('evaluationComment').value=t(ev.comment);$('fullAiJson').value='';toast('ChatGPT統合JSONを反映しました')}catch(e){console.error(e);alert('統合JSONの形式が正しくありません。')}}
